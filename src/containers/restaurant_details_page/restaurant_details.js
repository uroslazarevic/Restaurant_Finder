        ...
        if (scrollHeight < 10 && this.state.showNotification === false) {
          this.setState({ multiple: this.state.multiple + 1, reviewsLoader: true }, () => {
            ...
            getRestaurantReviews({
              res_id: this.props.location.state.resId,
              count: 5 * this.state.multiple,
            }).then(() =>
              this.setState({ reviewsLoader: false, showNotification: true }, () => {
          ...
          {showContent && (
            <div className="restaurant-details-content container">
              <div className="pathway-link container">
                ...
                <Link to={{ pathname: urlRestaurants.toLowerCase(), state: { cityName, cityId } }}>
                  Restaurants
                </Link>
                <span>
                  {' '}
...
export default connect(
  mapStateToProps,
  { getRestaurantDetails, getRestaurantReviews }
)(RestaurantDetails);
