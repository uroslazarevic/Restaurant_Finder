...
export default class CuisinesModal extends Component {
  ...
  handleInputValue(e) {
    this.setState({ searchInputValue: e.target.value }, () => {
      document.querySelector('.all-cuisines-container ul .cuisine-item') ? this.setState({ showModalMessage: false })
    ...
    return (
      <div className= "all-cuisines-container" onClick={this.props.handleCuisinesModal} >
        <div className="all-cuisine-content">
          <label >Select a cuisine<span className="hide-cuisine-modal" onClick={this.props.handleCuisinesModal} ><i className="fas fa-times"></i></span></label>
          <div className="search-cuisine">
            <input onChange={this.handleInputValue} value={this.state.searchInputValue} type="text" placeholder= "Start typing to search..." />
          </div>
          <ul className="all-cuisine-list">
            {this.props.allCuisines.map(cuisine => {
              ...
              <li 
                ...
                key={cuisine_id} 
                className="cuisine-item">{cuisine_name}</li> : null
            })}
