...
export default class LoginForm extends Component {
  ...
  setTagsInputValue(string) {
    if( string.length > this.state.createdTags.length ) {
      const newString = string.split(' ').map(word => word.replace('#', '')).map(tag => `#${tag}`).join(' ')
  ...
  addRes(place) {
    this.setState({ createdCollection: { ...this.state.createdCollection, restaurants: [...this.state.createdCollection.restaurants, place] } })
  }

  removeRes(place) {
    const restaurants = this.state.createdCollection.restaurants.filter( res => res.restaurant.id !== place.restaurant.id)
    this.setState({ createdCollection: { ...this.state.createdCollection, restaurants } })
  }

  handleInputState(event) {
    event.persist();
    const { searchProps: { debouncedSearchLocations, debouncedSearchRestaurants } } = this.props;
    const { value, name } = event.target;
    this.setState({ createdCollection: {...this.state.createdCollection, [name]: value  }},
      () => {
        const { createdCollection: { location, restaurant }, cityId } = this.state;
        // Set created tags
  ...
  autocompleteInput(cityId) {
    ...
    const name = event.target.dataset.autocomInput;
      this.setState({ cityId, createdCollection: {...this.state.createdCollection, [name]: value  } }, () => {
        ...
        .then(() => {
          // Search Restaurants
          restaurant.length > 1 && debouncedSearchRestaurants({ placeTerm: restaurant, entity_id: this.state.cityId })
        })
      })
    }
  }
  ...
  handleCollectionSaving() {
    ...
    const myCollection = { collection: { 
      ...
      image_url : `https://loremflickr.com/320/240/dish?random=${collection_id}`,
  ...
  componentWillMount() {
    ...
    this.props.searchProps.debouncedSearchLocations({ locationTerm: this.state.createdCollection.location });
  }
         ...
         <div className="actions">
           { showHiddenBtn &&
             <button
                ...
                data-ref-to= "addCollectionForm" >Back</button>
          }
          { !showHiddenBtn ? 
            <button
            ...
            :
            <button onClick = { this.handleCollectionSaving.bind(this) } className="save-collection">Save Collection</button>
          }
         </div>
        </div>
      </div>
